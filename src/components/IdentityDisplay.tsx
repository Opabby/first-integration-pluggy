import { useEffect, useState } from "react";
import {
  Box,
  Card,
  Text,
  Flex,
  Stack,
  Spinner,
  Badge,
} from "@chakra-ui/react";
import { pluggyApi } from "../services/pluggyApi";
import type { IdentityRecord } from "../types/pluggy";

interface IdentityDisplayProps {
  itemId: string;
}

const formatDate = (dateString?: string) => {
  if (!dateString) return "N/A";
  return new Date(dateString).toLocaleDateString("pt-BR");
};

const formatDocument = (document?: string, type?: string) => {
  if (!document) return "N/A";
  
  if (type === "CPF" && document.length === 11) {
    return document.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, "$1.$2.$3-$4");
  }
  
  if (type === "CNPJ" && document.length === 14) {
    return document.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5");
  }
  
  return document;
};

export const IdentityDisplay = ({ itemId }: IdentityDisplayProps) => {
  const [identity, setIdentity] = useState<IdentityRecord | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchIdentity = async () => {
      setIsLoading(true);
      setError(null);

      try {
        const data = await pluggyApi.getIdentityFromDb(itemId);
        setIdentity(data);
      } catch (err) {
        setError(
          err instanceof Error ? err.message : "Failed to load identity"
        );
      } finally {
        setIsLoading(false);
      }
    };

    if (itemId) {
      fetchIdentity();
    }
  }, [itemId]);

  if (isLoading) {
    return (
      <Flex justify="center" align="center" minH="200px">
        <Spinner size="xl" color="brand.500" />
      </Flex>
    );
  }

  if (error) {
    return (
      <Card.Root p={4}>
        <Text color="red.500">{error}</Text>
      </Card.Root>
    );
  }

  if (!identity) {
    return (
      <Card.Root p={8}>
        <Text textAlign="center" color="gray.500">
          No identity information available for this item.
        </Text>
      </Card.Root>
    );
  }

  return (
    <Card.Root p={6}>
      <Stack gap={6}>
        <Box>
          <Text fontSize="xl" fontWeight="bold" mb={4} color="brand.600">
            {identity.full_name ? "Personal Information" : "Company Information"}
          </Text>
          
          <Stack gap={3}>
            {identity.full_name && (
              <Flex justify="space-between">
                <Text fontWeight="medium" color="gray.600">Full Name:</Text>
                <Text>{identity.full_name}</Text>
              </Flex>
            )}

            {identity.company_name && (
              <Flex justify="space-between">
                <Text fontWeight="medium" color="gray.600">Company Name:</Text>
                <Text>{identity.company_name}</Text>
              </Flex>
            )}

            {identity.document && (
              <Flex justify="space-between">
                <Text fontWeight="medium" color="gray.600">
                  {identity.document_type || "Document"}:
                </Text>
                <Text>
                  {formatDocument(identity.document, identity.document_type)}
                </Text>
              </Flex>
            )}

            {identity.tax_number && (
              <Flex justify="space-between">
                <Text fontWeight="medium" color="gray.600">Tax Number:</Text>
                <Text>{identity.tax_number}</Text>
              </Flex>
            )}

            {identity.birth_date && (
              <Flex justify="space-between">
                <Text fontWeight="medium" color="gray.600">Birth Date:</Text>
                <Text>{formatDate(identity.birth_date)}</Text>
              </Flex>
            )}

            {identity.job_title && (
              <Flex justify="space-between">
                <Text fontWeight="medium" color="gray.600">Job Title:</Text>
                <Text>{identity.job_title}</Text>
              </Flex>
            )}
          </Stack>
        </Box>

        {(identity.emails && identity.emails.length > 0) ||
         (identity.phone_numbers && identity.phone_numbers.length > 0) ? (
          <Box>
            <Text fontSize="lg" fontWeight="bold" mb={3} color="brand.600">
              Contact Information
            </Text>
            
            <Stack gap={3}>
              {identity.emails && identity.emails.length > 0 && (
                <Box>
                  <Text fontWeight="medium" color="gray.600" mb={2}>Emails:</Text>
                  <Stack gap={2}>
                    {identity.emails.map((email, index) => (
                      <Flex key={index} gap={2} align="center">
                        {email.type && (
                          <Badge size="sm" colorScheme="blue">
                            {email.type}
                          </Badge>
                        )}
                        <Text>{email.value}</Text>
                      </Flex>
                    ))}
                  </Stack>
                </Box>
              )}

              {identity.phone_numbers && identity.phone_numbers.length > 0 && (
                <Box>
                  <Text fontWeight="medium" color="gray.600" mb={2}>Phone Numbers:</Text>
                  <Stack gap={2}>
                    {identity.phone_numbers.map((phone, index) => (
                      <Flex key={index} gap={2} align="center">
                        {phone.type && (
                          <Badge size="sm" colorScheme="green">
                            {phone.type}
                          </Badge>
                        )}
                        <Text>{phone.value}</Text>
                      </Flex>
                    ))}
                  </Stack>
                </Box>
              )}
            </Stack>
          </Box>
        ) : null}

        {identity.addresses && identity.addresses.length > 0 && (
          <Box>
            <Text fontSize="lg" fontWeight="bold" mb={3} color="brand.600">
              Addresses
            </Text>
            
            <Stack gap={3}>
              {identity.addresses.map((address, index) => (
                <Card.Root key={index} p={3} bg="gray.50">
                  {address.type && (
                    <Badge size="sm" colorScheme="purple" mb={2}>
                      {address.type}
                    </Badge>
                  )}
                  <Text fontSize="sm">
                    {address.fullAddress || address.primaryAddress || "N/A"}
                  </Text>
                  {(address.city || address.state || address.postalCode) && (
                    <Text fontSize="sm" color="gray.600" mt={1}>
                      {[address.city, address.state, address.postalCode]
                        .filter(Boolean)
                        .join(", ")}
                    </Text>
                  )}
                </Card.Root>
              ))}
            </Stack>
          </Box>
        )}

        {identity.relations && identity.relations.length > 0 && (
          <Box>
            <Text fontSize="lg" fontWeight="bold" mb={3} color="brand.600">
              Relations
            </Text>
            
            <Stack gap={2}>
              {identity.relations.map((relation, index) => (
                <Flex key={index} justify="space-between" p={2} bg="gray.50" borderRadius="md">
                  <Box>
                    <Text fontWeight="medium">{relation.name || "N/A"}</Text>
                    {relation.type && (
                      <Text fontSize="sm" color="gray.600">{relation.type}</Text>
                    )}
                  </Box>
                  {relation.document && (
                    <Text fontSize="sm" color="gray.600">
                      {relation.document}
                    </Text>
                  )}
                </Flex>
              ))}
            </Stack>
          </Box>
        )}
      </Stack>
    </Card.Root>
  );
};